	...
	<div class="card-body">
		<div class="row">
			...
			<div class="col-4 mb-5 text-end">
				...
				<?php } else { ?>
					<a id="change-customer" href="#" class="ms-1">
				...
				<div class="pull-right mt-4 text-end">
					...
					<?php foreach ($invoice_tax as $it) { ?>
						...
						<h7>
							...
							?>
							<a href="#" class="text-danger remove-tax" data-invoice-tax-id="<?php echo $it->itID; ?>">
	...
	<div class="card-body">
		<div class="row">
			<div class="col-12 mb-2">
				<label for="sel-tax"><?php echo lang('Text.inv_applicable_taxes') ?></label>
...
<script>
	$(document).ready(function() {
		...
		<?php } else { ?>
			$('#change-customer').on('click', function(e) {
				e.preventDefault();
				let customerID = $(this).val();
				const obj = {
					'field': 'customer',
		...
		$('#add-tax-invoice').on('click', function() {
			let taxID = $('#sel-tax').val();

			if (taxID != "") {
				$.ajax({
					type: "post",
					url: "<?php echo base_url('Invoice/addTaxToInvoice'); ?>",
					data: {
						'taxID': taxID,
						'invoiceID': "<?php echo $invoiceID; ?>"
					},
				...
				$('#sel-tax').addClass('is-invalid');
				Swal.fire({
					position: "top-end",
					icon: "warning",
					text: "<?php echo lang("Text.inv_action_add_tax_msg"); ?>..!",
		...
		$('.remove-tax').on('click', function(e) {
			e.preventDefault();

			let id = $(this).attr('data-invoice-tax-id');

			Swal.fire({
				title: '<?php echo lang('Text.are_you_sure_msg'); ?>',
				text: "<?php echo lang('Text.inv_remove_tax_msg'); ?>",
				icon: 'warning',
				showCancelButton: true,
				confirmButtonText: '<?php echo lang('Text.inv_yes_remove_tax'); ?>',
				cancelButtonText: '<?php echo lang('Text.no_cancel_msg'); ?>'
			}).then((result) => {
				if (result.isConfirmed) {
					$.ajax({
						type: "POST",
						url: "<?php echo base_url('Invoice/removeTaxInvoice'); ?>",
						data: {
							'id': id
						},
						dataType: "json",
